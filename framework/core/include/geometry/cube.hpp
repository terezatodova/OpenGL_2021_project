...
public:
    /**
     * Creates a cube whose center is in (0,0,0) and the length of its side is 2 units (positions of its vertices are
     * from -1 to 1).
     *
     ...
     */
    Cube(GLint position_loc = DEFAULT_POSITION_LOC, GLint normal_loc = DEFAULT_NORMAL_LOC,
         GLint tex_coord_loc = DEFAULT_TEX_COORD_LOC, GLint tangent_loc = DEFAULT_TANGENT_LOC,
         GLint bitangent_loc = DEFAULT_BITANGENT_LOC)
        : Geometry(GL_TRIANGLES, 14, cube_vertices_count, cube_vertices, cube_indices_count, cube_indices) {
    }
    ...
    void update(const glm::vec3& left_top_front, const glm::vec3& right_top_front, const glm::vec3& left_bottom_front,
        ...
        std::array<float, cube_vertices_count * 14> vertices =
            generate_custom_cube_vertices(left_top_front, right_top_front, left_bottom_front, right_bottom_front, left_top_back,
                                          right_top_back, left_bottom_back, right_bottom_back);
    ...
    void update(const glm::vec3 points[8]) {
        ...
        update(left_top_front, right_top_front, left_bottom_front, right_bottom_front, left_top_back, right_top_back,
